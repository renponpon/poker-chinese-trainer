...
- 過度に乱暴・失礼な表現は避ける
- 中国語は簡体字、ピンインは声調記号付き
- pinyin と examples の reading は音節ごとに半角スペースで区切る
...
- examples は phrase が中国語、reading が phrase 全文の声調記号付きピンイン、translation が日本語訳
- pinyin と examples の reading は中国語本文に対応する音だけにし、余計な文字・途中の断片・重複音を入れない
...
- 通常セクションは bullets に短い日本語箇条書きを1〜2個入れる
- 例を含む見出しは examples に必ず2つ入れ、bullets は空配列にする
- examples の phrase は学習対象言語の例文、reading は中国語なら声調記号付きピンイン・それ以外なら空文字、translation は日本語訳
