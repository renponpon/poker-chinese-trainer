...
- 1例目と2例目の間に1行空ける
- 通常の箇条書き内で中国語（簡体字）を書いた場合は、必ず直後に半角括弧で声調記号付きピンインを添える
- ただし【他の自然な言い方】【相手の想定返答】【返答するときの例】【類似・関連フレーズ】の例では、3行形式を優先し、括弧付きピンインを使わない
- 入力が名詞句や短い断片の場合、入力にない行動・意図を断定しない
- 必ず JSON のみを返す
